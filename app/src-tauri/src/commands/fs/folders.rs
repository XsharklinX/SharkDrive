...
pub async fn cmd_create_folder(
    ...
    index_state.upsert_folder(folder.clone());
    Ok(folder)
}
...
pub async fn cmd_delete_folder(
    ...
    index_state.remove_folder(folder_id);
    Ok(true)
}
...
pub async fn cmd_scan_folders(
    ...
    let cached_folder_map: std::collections::HashMap<i64, FolderMetadata> = index_state
        .get_folders()
        .into_iter()
        ...
        match &dialog.peer {
            Peer::Channel(c) => {
                ...
                state
                    ...
                    .lock()
                    .await
                    .insert(id.to_string(), dialog.peer.clone());
                ...
                if let Some(cached) = cached_folder_map.get(&id) {
                    if cached.name == strip_title_markers(&name) {
                        folders.push(cached.clone());
                        continue;
                    }
                }
    ...
    index_state.set_folders(folders.clone());
    Ok(folders)
}
...
pub async fn cmd_rename_folder(
    ...
    index_state.upsert_folder(FolderMetadata {
        id: folder_id,
        name: new_name,
        parent_id: index_state
            .get_folders()
            .into_iter()
