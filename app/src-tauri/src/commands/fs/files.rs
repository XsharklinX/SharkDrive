...
pub async fn cmd_search_global(
    ...
    let folder_name_map = index_state.folder_name_map();

    log::info!("Searching global for: {}", query);

    let dedup_key = |folder_id: Option<i64>, file_id: i64| -> String {
        match folder_id {
            Some(id) => format!("f{}:{}", id, file_id),
            None => format!("home:{}", file_id),
        }
    };

    let mut merged = std::collections::HashMap::<String, FileMetadata>::new();
    for file in index_state.get_all_files() {
        let folder_name = folder_name_map
            .get(&file.folder_id)
            .map(String::as_str)
            .unwrap_or("Saved Messages");
        if matches_search(&filters, &file, Some(folder_name), "") {
            merged.insert(dedup_key(file.folder_id, file.id), file);
        }
    }
    ...
    let mut push_message = |message: tl::types::Message| {
        if let Some(file) = raw_message_to_file_metadata(&message) {
            let folder_name = folder_name_map
                ...
                .map(String::as_str)
                .unwrap_or("Saved Messages");
            if matches_search(&filters, &file, Some(folder_name), &message.message) {
                merged.insert(dedup_key(file.folder_id, file.id), file);
            }
        }
    };
    ...
    files.extend(merged.into_values());
...
#[tauri::command]
pub async fn cmd_get_folder_size(
    folder_id: i64,
    state: State<'_, TelegramState>,
    index_state: State<'_, PersistentIndexState>,
) -> Result<(i64, u64), String> {
    // 1. Folder was opened — use the live file index
    let cached = index_state.get_files(Some(folder_id));
    if !cached.is_empty() {
        let count = cached.len() as i64;
        let bytes: u64 = cached.iter().map(|f| f.size).sum();
        return Ok((count, bytes));
    }

    // 2. Previously computed and cached
    if let Some((count, bytes)) = index_state.get_folder_size_cache(folder_id) {
        return Ok((count, bytes));
    }

    // 3. Fetch from Telegram and persist the result
    let client_opt = { state.client.lock().await.clone() };
    let client = client_opt.ok_or("Telegram client not connected".to_string())?;
    ...
    while let Some(msg) = msgs.next().await.map_err(|e| e.to_string())? {
        if let Some(media) = msg.media() {
            ...
            if let Media::Document(d) = media {
                total_bytes += d.size() as u64;
            }
        }
    }

    index_state.set_folder_size_cache(folder_id, count, total_bytes);
    Ok((count, total_bytes))
}

#[tauri::command]
pub async fn cmd_get_all_indexed_files(
    index_state: State<'_, PersistentIndexState>,
) -> Result<Vec<FileMetadata>, String> {
