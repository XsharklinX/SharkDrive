...
export function formatError(raw: unknown): string {
    ...
    if (floodMatch) {
        ...
        if (secs >= 3600) return `Telegram is limiting requests. Wait about ${Math.ceil(secs / 3600)} hour(s).`;
        if (secs >= 60) return `Telegram is limiting requests. Wait about ${Math.ceil(secs / 60)} minute(s).`;
        return `Telegram is limiting requests. Wait ${secs} second(s).`;
    }

    if (lower.includes('not connected') || lower.includes('client not connected') || lower.includes('disconnected'))
    ...
    if (lower.includes('bandwidth limit'))
        return 'Daily bandwidth limit reached (250 GB). Try again tomorrow.';

    if (lower.includes('encryption key not loaded') || lower.includes('encryption password'))
        return 'Load your encryption password in Settings before downloading this file.';

    if (msg.includes('PEER_ID_INVALID') || lower.includes('not found'))
        return 'File or folder not found. It may have been deleted from Telegram.';

    if (msg.includes('AUTH_KEY_UNREGISTERED') || lower.includes('session expired'))
        return 'Your Telegram session expired. Please sign in again.';

    if (lower.includes('network') || lower.includes('socket') || lower.includes('eof') || lower.includes('connection'))
        return 'Network connection failed. Check Telegram connectivity and try again.';
