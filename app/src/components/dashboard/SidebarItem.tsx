...
function FolderContextMenu({ x, y, isEncrypted, onOpen, onRename, onShareLink, onToggleEncryption, onCreateChild, onMoveToRoot, onMoveFolderTo, onDelete, onClose }: FolderContextMenuProps) {
    ...
    return (
        ...
        >
            ...
            {onMoveFolderTo && (
                <button onClick={onMoveFolderTo} className={`${btn} text-telegram-text`}>
        ...
        <>
            ...
            >
                ...
                <span className="flex-1 text-left truncate">
                    {isOver ? <span className="text-telegram-primary font-semibold">Move here</span> : label}
                </span>
                {!isOver && fileCount !== undefined && fileCount > 0 && (
                    <span className="shrink-0 text-[10px] tabular-nums text-telegram-subtext/60">
            ...
            {contextMenu && hasContextMenu && (
                <FolderContextMenu
                    ...
                    onMoveToRoot={onMoveToRoot ? () => { onMoveToRoot(); setContextMenu(null); } : undefined}
