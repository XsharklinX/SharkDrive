...
interface FileExplorerProps {
    ...
    onInlineRename?: (file: TelegramFile, newName: string) => void;
    onShareLink: (file: TelegramFile) => void;
    onCopyToFolder?: (file: TelegramFile) => void;
...
export function FileExplorer({
    ...
    const didLoadSavedControls = useRef(false);
    ...
    useEffect(() => {
        try {
            const saved = localStorage.getItem(EXPLORER_CONTROLS_KEY);
            if (!saved) return;
            const parsed = JSON.parse(saved) as Partial<{ sortField: SortField; sortDirection: SortDirection; filterType: FilterType }>;
            if (parsed.sortField && ['name', 'size', 'date'].includes(parsed.sortField)) setSortField(parsed.sortField);
            if (parsed.sortDirection && ['asc', 'desc'].includes(parsed.sortDirection)) setSortDirection(parsed.sortDirection);
            if (parsed.filterType && ['all', 'image', 'video', 'audio', 'doc', 'other'].includes(parsed.filterType)) setFilterType(parsed.filterType);
        } catch {
            localStorage.removeItem(EXPLORER_CONTROLS_KEY);
        } finally {
            didLoadSavedControls.current = true;
        }
    }, []);

    useEffect(() => {
        if (!didLoadSavedControls.current) return;
        localStorage.setItem(EXPLORER_CONTROLS_KEY, JSON.stringify({ sortField, sortDirection, filterType }));
    }, [sortField, sortDirection, filterType]);

    useEffect(() => {
        if (selectedIds.length === 0) lastSelectedIndexRef.current = null;
    }, [selectedIds]);
    ...
    const handleRangeToggle = useCallback((id: number, index: number, extendRange: boolean) => {
        if (extendRange && lastSelectedIndexRef.current !== null) {
    ...
    }, [sortedFiles, onToggleSelection, onSelectRange]);
        ...
        return (
            <div className="grid flex-1 content-start gap-3 p-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                {Array.from({ length: 12 }).map((_, index) => (
                    <div key={index} className="h-32 animate-pulse rounded-lg border border-telegram-border bg-white/[0.025]" />
                ))}
            </div>
        );
    }
    ...
    if (files.length === 0) {
        return (
            <div className="flex-1 overflow-auto p-6">
                <EmptyState onUpload={onManualUpload} variant={emptyVariant} searchTerm={searchTerm} />
            </div>
        );
    }
                        ...
                        return (
                            ...
                            >
                                {row.map((item, colIndex) => {
                                    ...
                                    const fileIndex = virtualRow.index * columns + colIndex;
                                    return (
                                        <FileCard
                                            key={item.id}
                                            file={item}
                                            ...
                                            onToggleSelection={(event) => handleRangeToggle(item.id, fileIndex, event.shiftKey)}
                            ...
                            return (
                                ...
                                >
                                    <FileListItem
                                        ...
                                        selectionMode={selectionMode}
                                        onFileClick={(e, id) => {
                                            handleRangeToggle(id, virtualItem.index, e.shiftKey);
                                        }}
